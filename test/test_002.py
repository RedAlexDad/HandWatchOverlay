        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if selected_watch is not None:
                    ...
                    middle_finger_x = int(hand_landmarks.landmark[12].x * frame.shape[1])
                    middle_finger_y = int(hand_landmarks.landmark[12].y * frame.shape[0])

                    wrist_to_finger_dist = abs(wrist_x - middle_finger_x)
                    # Отрегулируйте коэффициент масштабирования по мере необходимости
                    watch_width = 2 * (wrist_to_finger_dist // 2)
                    watch_height = 2 * (watch_width * watch_img.shape[0] // watch_img.shape[1] // 2)

                    watch_width = max(1, watch_width)
                    watch_height = max(1, watch_height)
                    ...
                    # Убедитесь, что размеры overlay_region и resized_watch совпадают
                    overlay_region_resized = cv2.resize(overlay_region, (watch_width, watch_height),
                                                        interpolation=cv2.INTER_AREA)

                    resized_watch = resized_watch[:watch_height, :watch_width]

                    wrist_x = max(0, min(wrist_x, frame.shape[1] - 1))
                    wrist_y = max(0, min(wrist_y, frame.shape[0] - 1))

                    # Наложение часов с использованием альфа-наложения
                    frame[wrist_y - watch_height // 2:wrist_y + watch_height // 2,
                    wrist_x - watch_width // 2:wrist_x + watch_width // 2] = cv2.addWeighted(overlay_region_resized,
                                                                                             alpha,
