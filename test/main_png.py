...
class HandWatchOverlay:
    ...
    def resize_watch(self, base_width, base_height, scale_factor=1.0):
        ...
        if scale_factor < 0:
            ...
            return cv2.resize(self.clock_image, (width, height), interpolation=cv2.INTER_AREA)  # Use INTER_AREA
        else:
            return cv2.resize(self.clock_image, (base_width, base_height), interpolation=cv2.INTER_AREA)  # Use INTER_AREA
    ...
    def overlay_watch(self, frame, clock_resized, wrist_x, wrist_y, width, height):
        ...
        # Получение альфа-канала изображения часов
